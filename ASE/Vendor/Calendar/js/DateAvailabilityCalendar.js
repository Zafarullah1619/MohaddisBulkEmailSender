function InitializeAvailability(CalendarSelector, Q, curr, NextDates) {

    InitializeQuarterCalendar(CalendarSelector, 4, null, Q, curr, NextDates);
}

function InitializeAvailabilityCalendar(CalendarSelector, SelectedList, Q, curr, NextDates) {
    //alert("Curr: " + curr);
    InitializeQuarterCalendar(CalendarSelector, 4, SelectedList, Q, curr, NextDates);
}
function InitializeQuarterCalendar(CalendarSelector, Quarters, SelectedList, Q, curr, NextDates) {
    //console.log("Hello Date " + SelectedList);
    //console.log("Q "+Q);
    //console.log(SelectedList[0][0].Duration);
    /**************** Start Making Calender***********************/
    if (SelectedList == null || SelectedList[0][0].Duration != Q) {
        ...
        //console.log(SelectedList[0][0].EndTimeMinute);
    }

    
    //console.log(SelectedList[0][0].Duration);
    //console.log(Q);
    //console.log(SelectedStartQuarter);
    ...
    //curr = moment(curr).format($("#MomentDateFormat").val());
    console.log("curr Week start: " + curr);
    if (NextDates == "yes") {
        StartDate = moment(moment(curr, "DD-MMM-YYYY")).format("DD-MMM-YYYY");
        SecondDate = moment(moment(curr, "DD-MMM-YYYY").add('days', 1)).format("DD-MMM-YYYY");
        ThirdDate = moment(moment(curr, "DD-MMM-YYYY").add('days', 2)).format("DD-MMM-YYYY");
        FourthDate = moment(moment(curr, "DD-MMM-YYYY").add('days', 3)).format("DD-MMM-YYYY");
        FifthDate = moment(moment(curr, "DD-MMM-YYYY").add('days', 4)).format("DD-MMM-YYYY");
        SixthDate = moment(moment(curr, "DD-MMM-YYYY").add('days', 5)).format("DD-MMM-YYYY");
        SeventhDate = moment(moment(curr, "DD-MMM-YYYY").add('days', 6)).format("DD-MMM-YYYY");
        NewStartDate = moment(moment(curr, "DD-MMM-YYYY").add('days', 7)).format("DD-MMM-YYYY");
        OldStartDate = moment(moment(curr, "DD-MMM-YYYY")).format("DD-MMM-YYYY");
        NextStartDate = moment(moment(curr, "DD-MMM-YYYY")).format("DD-MMM-YYYY");
        PreStartDate = moment(moment(curr, "DD-MMM-YYYY").subtract(7, "days")).format("DD-MMM-YYYY");
    }
    else {
        NextStartDate = moment(moment(curr, "DD-MMM-YYYY").subtract(7, "days")).format("DD-MMM-YYYY");
        StartDate = moment(moment(curr, "DD-MMM-YYYY").subtract(7, "days")).format("DD-MMM-YYYY");
        SecondDate = moment(moment(curr, "DD-MMM-YYYY").subtract(6, "days")).format("DD-MMM-YYYY");
        ThirdDate = moment(moment(curr, "DD-MMM-YYYY").subtract(5, "days")).format("DD-MMM-YYYY");
        FourthDate = moment(moment(curr, "DD-MMM-YYYY").subtract(4, "days")).format("DD-MMM-YYYY");
        FifthDate = moment(moment(curr, "DD-MMM-YYYY").subtract(3, "days")).format("DD-MMM-YYYY");
        SixthDate = moment(moment(curr, "DD-MMM-YYYY").subtract(2, "days")).format("DD-MMM-YYYY");
        SeventhDate = moment(moment(curr, "DD-MMM-YYYY").subtract(1, "days")).format("DD-MMM-YYYY");
        //NextStartDate = moment(moment(curr, "DD-MMM-YYYY")).format("DD-MMM-YYYY");
        PreStartDate = moment(moment(curr, "DD-MMM-YYYY").subtract(7, "days")).format("DD-MMM-YYYY");
        OldStartDate = moment(moment(curr, "DD-MMM-YYYY").subtract(7, "days")).format("DD-MMM-YYYY");
        NewStartDate = moment(moment(curr, "DD-MMM-YYYY")).format("DD-MMM-YYYY");
    }
    /*
    ...
    console.log("FourthDate: " + FourthDate);
    console.log("FifthDate: " + FifthDate);
    console.log("SixthDate: " + SixthDate);
    console.log("SeventhDate: " + SeventhDate);

    console.log("OldStartDate: " + OldStartDate);
    console.log("FifthDate: " + FifthDate);
    console.log("NextStartDate: " + NextStartDate);
    console.log("PreStartDate: " + PreStartDate);
    */
    //var HiddenId = StartDate;
    //if (!$("#" + HiddenId).length) {
    //    var $hiddenInput = $('<input/>', { type: 'hidden', id: HiddenId, value: HiddenId, class: 'SlotsHiddenField' });
    ...
    $(CalendarSelector + " .LabelRow").append('<div class="col-sm-1 daysRow LabelCell"><strong>' + StartDate + '</strong></div>');
    $(CalendarSelector + " .LabelRow").append('<div class="col-sm-1 daysRow LabelCell"><strong>' + SecondDate + '</strong></div>');
    $(CalendarSelector + " .LabelRow").append('<div class="col-sm-1 daysRow LabelCell"><strong>' + ThirdDate + '</strong></div>');
    $(CalendarSelector + " .LabelRow").append('<div class="col-sm-1 daysRow LabelCell"><strong>' + FourthDate + '</strong></div>');
    $(CalendarSelector + " .LabelRow").append('<div class="col-sm-1 daysRow LabelCell"><strong>' + FifthDate + '</strong></div>');
    $(CalendarSelector + " .LabelRow").append('<div class="col-sm-1 daysRow LabelCell"><strong>' + SixthDate + '</strong></div>');
    $(CalendarSelector + " .LabelRow").append('<div class="col-sm-1 daysRow LabelCell"><strong>' + SeventhDate + '</strong></div>');
    ...
    $(CalendarSelector + " .TimeListing").html('<i class="fa fa-arrow-circle-o-left PreviousDates" data-PreStartDate=' + OldStartDate + ' data-NextStartDate=' + OldStartDate + '></i> <i class="fa fa-arrow-circle-o-right NextDates" data-PreStartDate=' + NextStartDate + ' data-NextStartDate=' + NewStartDate + '></i>');
    ...
    for (HourRow = 1; HourRow <= 24; HourRow++) {
        for (QuarterRow = 0; QuarterRow < 4; QuarterRow = QuarterRow + (AddCount)) {
            ...
            for (var i = 1; i <= 7; i++) {
                ...
                var DisabledCellClass = "";
                //if (i == "1" || i==1)
                //{
                //    DisabledCellClass = " DisabledCell ";
                //    SelectedCellClass = "";
                //}
                Data += '<div class="col-sm-1 TimeCell ' + SelectedCellClass + TimeClass + DisabledCellClass + '" data-StartTimeHour = "' + StartTimeHour + '" data-StartTimeMinute = "' + StartTimeMinute + '" data-EndTimeHour = "' + EndTimeHour + '" data-EndTimeMinute = "' + EndTimeMinute + '" data-row="' + RowCount + '" data-col="' + i + '"></div>';
    ...
    $(document).on("mouseup", CalendarSelector + " .TimeCell", function () {
        ...
        $(CalendarSelector + " .SelectedHover").each(function () {
    ...
    $(document).on("mouseenter", CalendarSelector + " .TimeCell", function () {
        ...
        if (StartColNumber > 0 && EndColNumber > 0 && StartRowNumber > 0 && EndRowNumber > 0 && RowDifference > 0 && ColDifference > 0) {
            $(CalendarSelector + " .SelectedHover").removeClass("SelectedHover");
            for (var rowNo = StartRowNumber; rowNo <= EndRowNumber; rowNo = rowNo + RowDifference) {
                ...
                for (var colNo = StartColNumber; colNo <= EndColNumber; colNo = colNo + ColDifference) {
                    ...
                    var CurrentCell = $(CalendarSelector + " .TimeCell[data-row='" + rowNo + "'][data-col='" + colNo + "']");
...
function GetDayTimes(CalendarSelector, DayOfWeek) {
    ...
    $(CalendarSelector + " .SelectedCell[data-col=" + DayOfWeek + "]").each(function () {
        ...
        var WStartDate = $("#hdnCurrentWeekDate").val();
        //var DurationSlot = $("#Duration").attr("data-slots");
        var DurationSlot = $('select#Duration option:selected').attr('data-slots');
        //console.log(DurationSlot);
        Times.push(
            {
                key: RowValue,
